...
test('eager loading supports primary key type', function (string $modelClass, string $factoryClass): void {
    ...
    $lazyFirst  = $first->settings()->all()->sortKeys()->all();
    $lazySecond = $second->settings()->all()->sortKeys()->all();

    expect($first->relationLoaded('modelSettings'))->toBeFalse();
    expect($second->relationLoaded('modelSettings'))->toBeFalse();
    ...
    expect($first->settings()->all()->sortKeys()->all())->toBe($lazyFirst);
    expect($second->settings()->all()->sortKeys()->all())->toBe($lazySecond);
    ...
    'string'  => [SomeString::class, SomeStringFactory::class],
