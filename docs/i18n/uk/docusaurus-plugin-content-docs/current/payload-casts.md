...
Без користувацького перетворення пакет кодує кожне значення у JSON під час запису й повертає точне
декодоване JSON-значення під час читання. Це стосується `null`, порожніх рядків, рядків із пробілів,
порожніх масивів, нуля та `false`.
...
конструктора можуть використовувати звичайні прив’язки контейнера. Користувацький `set()` отримує
кожне вхідне значення, включно зі значеннями, які Laravel вважає порожніми.
...
- [Робота з налаштуваннями](settings.md) — як точні JSON-значення зберігаються й видаляються.
