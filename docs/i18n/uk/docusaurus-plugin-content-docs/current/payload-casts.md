...
Користувацькі перетворення налаштовуються за класом батьківської моделі:
...
Одне налаштоване перетворення обробляє дані всіх налаштувань цього класу батьківської моделі. Перед
вибором перетворення псевдоніми Laravel morph map розв’язуються назад у клас моделі.

Налаштований клас має реалізовувати `CastsAttributes` або розширювати `Spatie\LaravelData\Data`.
Для інших класів спеціальна обробка не застосовується, і значення використовують стандартний шлях JSON.
...
чи `Post`. Пакет створює екземпляр перетворення без аргументів конструктора.
...
Перетворення вибирається для класу батьківської моделі, а не для окремого ключа. Тому дані кожного
налаштування цієї моделі мають бути допустимими вхідними даними для налаштованого перетворення.
