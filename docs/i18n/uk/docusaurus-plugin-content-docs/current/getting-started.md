...
збереження батьківської моделі. Для незбереженої моделі `settings()->get()` повертає `null`, а
`settings()->all()` — порожню колекцію, навіть якщо для класу є значення за замовчуванням. Кожен
метод зміни викидає
