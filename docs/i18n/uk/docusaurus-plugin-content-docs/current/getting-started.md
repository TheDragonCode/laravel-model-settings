...
Викликайте `settings()->set()` лише після збереження батьківської моделі. Незбережена модель не має
первинного ключа. Її `settings()->get()` повертає `null`, а `settings()->all()` — порожню колекцію,
навіть якщо для класу є значення за замовчуванням.
