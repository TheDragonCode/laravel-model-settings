...
ідентифікатори, UUID і ULID. Для довшого користувацького первинного ключа потрібна відповідна зміна
міграції.

Значення `0` в `item_id` зарезервоване для налаштувань класу за замовчуванням. Якщо дані вже існують,
...
| Перетворювати `item_id` на `string` | Цілі числа, UUID і ULID зберігаються в одному стовпці |
