...
`null`을 반환하고 `all()`은 모델 재정의를 쿼리하지 않고 빈 컬렉션을 반환합니다. `set()`과 `forget()`은
저장소 쿼리가 실행되기 전에 `InvalidSettingsOwnerException`을 발생시킵니다.

정수 `0`과 문자열 `'0'`은 1.x에서 공유 기본값을 위해 예약되어 있습니다. 이 키를 가진 저장된 모델은 클래스
기본값을 읽을 수 있지만 `set()`과 `forget()`은 `InvalidSettingsOwnerException`을 발생시킵니다. `'00'`을
포함한 다른 문자열 키는 계속 유효합니다.
