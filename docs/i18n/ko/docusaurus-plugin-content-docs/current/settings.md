...
`get()`, `has()`, `all()`은 같은 우선순위로 값을 결정합니다.
...
대체값은 받지 않습니다. `has()`는 키가 없는 경우와 저장된 JSON `null`을 구분합니다.
...
저장 키로 정규화되면 마지막 값이 적용됩니다. 모든 값이 저장되며 행은 `forget()`과 `forgetMany()`만
삭제합니다.

비어 있지 않은 `setMany()` 일괄 작업은 트랜잭션 안에서 데이터베이스 네이티브 upsert 한 번을 사용합니다.
`forgetMany()`는 나열된 모든 키를 한 번의 삭제로 처리합니다. 쿼리 수는 키 수가 아니라 연산 유형에 따라
제한됩니다.
...
`set()`과 `setMany()`는 JSON 값을 정확히 저장합니다.
...
저장된 `null`은 존재하는 값으로 간주합니다. `has($key)`는 `true`를 반환하고 `get($key)`는 `null`을
반환합니다. 값이 `null`인 모델 재정의는 `forget()`으로 제거할 때까지 채워진 클래스 기본값도 숨깁니다.
...
빈 키와 공백만 있는 키는 `DragonCode\LaravelModelSettings\Exceptions\InvalidSettingKey`를 발생시킵니다.
정수와 enum을 저장 문자열로 정규화한 뒤 검증합니다. 예외 메시지에는 거부된 키나 설정 페이로드가 포함되지
않습니다.
...
`null`, `has()`는 `false`를 반환하고 `all()`은 모델 재정의를 쿼리하지 않고 빈 컬렉션을 반환합니다.
`set()`, `setMany()`, `forget()`, `forgetMany()`, `purge()`는 저장소 쿼리나 iterable 소비 전에
`InvalidSettingsOwnerException`을 발생시킵니다.

정수 식별자 `0` 또는 문자열 `'0'`을 가진 저장된 모델은 다른 저장된 소유자와 동일한 읽기 및 변경 작업을
지원합니다. 범위 판별자가 재정의와 클래스 기본값을 분리하므로 두 행이 모두 `item_id = '0'`을 유지해도
충돌하지 않습니다. `'00'`을 포함한 다른 문자열 키는 계속 유효합니다.
