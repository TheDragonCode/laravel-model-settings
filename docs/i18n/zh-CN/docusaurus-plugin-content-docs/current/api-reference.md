...
| `has(int\|string\|UnitEnum $key)` | `bool` | 报告最终键是否存在，包括已存储的 `null` |
| `set(int\|string\|UnitEnum $key, mixed $value)` | `void` | 使用准确的 JSON 值创建或替换设置 |
| `setMany(iterable $values)` | `void` | 在一个有限的事务批次中 upsert 所有值 |
...
`SettingsService` 的 `get()` 没有调用方提供的回退值参数。请使用 `has($key)` 区分最终键不存在和已存储的
JSON `null`。

## 解析矩阵

| 模型覆盖值 | 类默认值 | `get()` 结果 | `has()` 结果 | 是否包含在 `all()` 中 |
|------------|----------|--------------|--------------|-------------------------|
| 存在 | 存在 | 覆盖值，包括 `null` | `true` | 覆盖值 |
| 存在 | 不存在 | 覆盖值，包括 `null` | `true` | 覆盖值 |
| 不存在 | 存在 | 默认值，包括 `null` | `true` | 默认值 |
| 不存在 | 不存在 | `null` | `false` | 无记录 |

对于未保存的模型，`get()` 返回 `null`，`has()` 返回 `false`，`all()` 返回空集合。只有已持久化的模型才会
继承类默认值。
...
存在模型覆盖记录或类默认值记录时，此方法返回 `true`。已存储的 JSON `null` 返回 `true`；缺失的键返回
`false`。延迟加载和预加载服务采用相同的优先级，预加载路径不会执行额外的设置查询。
...
此方法先验证所有者和规范化后的键，然后按模型类型、模型标识符、作用域判别字段和键执行
update-or-create。所有 JSON 值都会被存储，包括 `null`、空字符串、仅含空白字符的字符串、空数组、零和
`false`。写入成功后，已加载的 `modelSettings` 关联会被清除，确保下一次读取不会复用过期数据。
...
iterable 的键按与 `set()` 相同的方式规范化。多个输入键规范化为同一个字符串时，最后一个值生效。所有值
都会在一个事务中使用一次数据库原生 upsert。此方法在读取 iterable 前验证所有者，并在成功后清除一次
`modelSettings`。删除请使用 `forgetMany()`。
...
`defaultSettings()` 返回的服务提供相同的八个方法：
...
默认值作用域。只读访问保持确定：未保存的所有者不查询覆盖值并返回 `null` 或空集合，且 `has()` 返回
`false`。主键为整数 `0` 或字符串 `'0'` 的已持久化所有者可以读取和修改自己的覆盖值；`is_default` 会
将这些记录与类默认值分开。
