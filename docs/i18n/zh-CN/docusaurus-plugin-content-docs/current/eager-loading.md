...
延迟读取设置时会加载 `modelSettings` 关联。对于集合，这会为每个模型额外执行一次设置查询。
...
预加载的关联包含每个模型的覆盖值及其继承的所有默认值。后续的 `get()` 和 `all()` 调用会使用已加载关联。
...
整数、UUID 和 ULID 主键都已覆盖此行为。

## 预加载后的修改

`set()` 和 `forget()` 会清除对应模型上已加载的 `modelSettings` 关联。下一次读取会重新加载该关联，因此不会
返回过期值。
