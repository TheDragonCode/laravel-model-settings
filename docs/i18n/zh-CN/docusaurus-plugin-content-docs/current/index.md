...
需要判断最终键是否存在时（包括存储值为 JSON `null` 的情况），请使用 `has()`。`get()` 有意不接受
调用方提供的回退值参数：唯一的回退值是持久化的类默认值；两个作用域都没有该键时返回 `null`。

默认值和覆盖值使用相同的操作：`all()`、`get()`、`has()`、`set()`、`setMany()`、`forget()`、
`forgetMany()` 和 `purge()`。
...
| `item_id` | 父模型主键；类默认值保留物理值 `0` |
| `is_default` | 类默认值为 `true`，模型覆盖值为 `false` |
| `key` | 设置名称 |
...
此软件包支持使用整数、字符串、UUID 或 ULID 主键的 Eloquent 模型。标识符为整数 `0` 或字符串 `'0'` 的
已持久化模型也能存储覆盖值，而不会与类默认值冲突。模型也可以使用 Laravel morph map。

单个模型的设置只属于已持久化的模型。未保存的模型不会继承默认值：`get()` 返回 `null`，`has()` 返回
`false`，`all()` 返回空集合。对未保存的所有者调用 `set()`、`setMany()`、`forget()`、`forgetMany()` 或
`purge()` 时，会在执行存储查询前抛出 `InvalidSettingsOwnerException`。

设置数据以 JSON 格式存储，包括 `null`、空字符串、仅含空白字符的字符串、空数组、零和 `false`。未配置
转换时，读取操作会返回准确的解码值。[数据转换](payload-casts.md)可以改为返回应用程序专用对象。
