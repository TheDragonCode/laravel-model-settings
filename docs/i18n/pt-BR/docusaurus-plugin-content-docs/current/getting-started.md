...
modelo pai for persistido. Para um modelo não persistido, `settings()->get()` retorna `null`,
`settings()->has()` retorna `false` e `settings()->all()` retorna uma coleção vazia mesmo quando a
classe tem valores padrão. Cada método de alteração lança
