...
Вызывайте `settings()->set()` и `settings()->forget()` только после сохранения родительской модели.
Для несохранённой модели `settings()->get()` возвращает `null`, а `settings()->all()` — пустую
коллекцию, даже если для класса есть значения по умолчанию. Оба изменяющих метода выбрасывают
