...
Вызывайте `settings()->set()` только после сохранения родительской модели. У несохранённой модели
нет первичного ключа. Её `settings()->get()` возвращает `null`, а `settings()->all()` — пустую
коллекцию, даже если для класса есть значения по умолчанию.
