...
сохранения родительской модели. Для несохранённой модели `settings()->get()` возвращает `null`,
`settings()->has()` — `false`, а `settings()->all()` — пустую коллекцию, даже если для класса есть
значения по умолчанию. Каждый изменяющий метод выбрасывает
