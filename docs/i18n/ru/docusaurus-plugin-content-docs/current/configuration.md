...
Значение `0` в `item_id` зарезервировано для настроек класса по умолчанию. В версии 1.x методы
`set()` и `forget()` отклоняют сохранённого владельца с целочисленным ключом `0` или строковым ключом
`'0'`, выбрасывая `InvalidSettingsOwnerException` до запроса к этой таблице. Если данные уже
существуют, изменение подключения, имени таблицы или псевдонимов morph map требует самостоятельного
переноса или обновления строк.
