...
const config = await readFile(path.join(siteDirectory, "docusaurus.config.ts"), "utf8");
...
for (const locale of locales.filter((locale) => locale !== defaultLocale)) {
    ...
    const missingFiles = sourceFiles.filter((file) => !localizedFiles.includes(file));
    const extraFiles = localizedFiles.filter((file) => !sourceFiles.includes(file));
    ...
    for (const file of sourceFiles.filter((file) => localizedFiles.includes(file))) {
        ...
        if (source === localized) {
            errors.push(`${locale}: ${file} is identical to the English source`);
        }
    }
}

if (errors.length > 0) {
    throw new Error(`Incomplete documentation localization:\n${errors.map((error) => `- ${error}`).join("\n")}`);
}

console.log(`Documentation localization is complete for ${locales.length - 1} locales and ${sourceFiles.length} pages.`);
