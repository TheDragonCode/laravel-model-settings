...
model has been saved. For an unsaved model, `settings()->get()` returns `null`, and
`settings()->all()` returns an empty collection even when class defaults exist. Every mutation method
throws
